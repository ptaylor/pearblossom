...
- Node geometry: Picasa stores `(x, y)` as the photo's top-left corner with a `theta` rotation. The importer negates `theta` (Pearblossom positive rotation = clockwise) and stores the center as `topLeft + R_cw(theta) · (w/2, h/2)` to reproduce Picasa's layout.
