...
- Node geometry: Picasa stores `(x, y)` as the photo's top-left corner and `theta` as a clockwise rotation around that corner; the importer converts this into Pearblossom's center-based `position`/`rotation` (`center = topLeft + R_cw(theta) · (w/2, h/2)`).
